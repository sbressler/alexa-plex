...
function appPre(request, response, type) {
    if(process.env.ALEXA_APP_ID) {
        // The app ID format has evolved and now can be prefixed by one of a few strings -- allow any
        expectedAppIds = [ "amzn1.echo-sdk-ams.app." + process.env.ALEXA_APP_ID,
                           "amzn1.ask.skill." + process.env.ALEXA_APP_ID];
        if (expectedAppIds.indexOf(request.sessionDetails.application.applicationId) < 0) {
            // Fail silently
